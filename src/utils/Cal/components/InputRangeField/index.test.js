...
describe("InputRangeField tests", () => {
  ...
  test("Should rerender when props change", () => {
    ...
    expect(wrapper.find(`.${styles.inputRangeInput}`).prop("value")).toEqual(
      12
    );
    expect(
      wrapper.find(`.${styles.inputRangeInput}`).prop("placeholder")
    ).toEqual("Placeholder");
    expect(wrapper.find(`.${styles.inputRangeLabel}`).text()).toEqual(
      "Input label"
    );
  ...
  test("Should render the label as a Component", () => {
    const Label = () => (
      <span className="input-range-field-label">Input label</span>
    );
    const wrapper = mount(
      <InputRangeField
        value={12}
    ...
    expect(wrapper.find(`.input-range-field-label`).text()).toEqual(
      "Input label"
    );
