    ...
    <div className="flex justify-center items-center px-4 md:px-20 relative h-screen">
      <Card className="rounded-sm py-5 px-4 md:px-12 w-full max-w-[900px] !shadow-none">
        <div className="space-y-2 flex justify-center flex-col items-center">
          <p className="text-center font-medium text-[1rem] text-[#6C6C6C] !mt-3">
            {screen === 1 ? 'Welcome to glitbase' : 'What you can do'}
          </p>
          <Typography
            variant="heading"
            className={`!leading-[46px] text-center !text-[2rem] font-medium font-[lora] mx-auto ${
              screen === 1 ? '!max-w-[400px]' : '!max-w-[500px]'
            }`}
          >
            {screen === 1
        ...
        {screen === 1 ? (
          <div className="w-[300px] mx-auto my-10 relative">
            <img
              src={StarsPink}
              alt="stars"
              className="absolute md:-left-44 -left-32 bottom-4"
            />
            <img src={SplashImg} alt="splash_img" />
            <img
              src={StarsGreen}
              alt="stars"
              className="absolute md:-right-44 -right-32 top-4"
            />
          </div>
        ) : (
        ...
        <div className="flex justify-center">
          <Button
            variant="default"
            onClick={
              screen === 1 ? () => setScreen(2) : () => navigate('signup')
            }
      ...
      <img
        src={StarsIllus}
        alt="star_illus"
        className="absolute bottom-0 transform scale-80"
      />
    </div>
  );
};
