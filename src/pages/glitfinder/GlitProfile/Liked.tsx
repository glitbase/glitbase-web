...
import { useGetMyLikedGlitsQuery } from '@/redux/glits';
...
export default function Liked({ profileData }: LikedProps) {
  const navigate = useNavigate();
  const isMdUp = useMatchMedia('(min-width: 768px)');
  const columnCount = isMdUp ? COLUMN_COUNT_MD_UP : COLUMN_COUNT_MOBILE;

  const { data, isLoading, isError, refetch } = useGetMyLikedGlitsQuery({ page: 1, limit: 50 });
  ...
  if (isError) {
    return (
      <div className="flex flex-col items-center justify-center py-16 px-4">
        <p className="text-[16px] text-[#DC2626] mb-2">Failed to load liked glits</p>
  ...
  if (glits.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-20 px-8">
        <h2 className="text-[20px] font-semibold text-[#101828] font-[lora] mb-2 tracking-tight">Nothing liked yet</h2>
        <p className="text-[16px] text-[#6C6C6C] font-medium text-center max-w-[360px]">
          Browse Glitfinder and tap the heart on looks you want to keep in your collection
        </p>
      </div>
    );
  }
